...
if __name__ == "__main__":
    ...
    try:
        ...
        conditions = [
            ...
            df_rpog['OVERLAP_TYPE'].isin(['Riparian/OGDA overlap', 'Riparian/IDF/OGDA overlap'])
        ]
        ...
        df_rpog_sum = df_rpog.groupby(['TSA_NAME', 'OVERLAP_TYPE_2'])[['THLB_AREA_DECREASE']].sum().reset_index()
        ...
        conditions = [
            ...
            df_rpdf['OVERLAP_TYPE'].isin(['Riparian/IDF overlap', 'Riparian/IDF/OGDA overlap'])
        ]
        
        values = ['IDF only', 'Riparian only', 'Riparian/IDF overlap']
        ...
        df_rpdf_okn = df_rpdf_okn[
            (df_rpdf_okn['OVERLAP_TYPE_2'] != 'IDF only') | 
        ...
        df_rpdf_okn_s1['REDUCTION_FACTOR_S1'] = np.where(
            df_rpdf_okn_s1['OVERLAP_TYPE_2'] != 'IDF only', 
            1, 
        ...
        df_rpdf_okn_s1_sum = df_rpdf_okn_s1.groupby(['TSA_NAME', 'OVERLAP_TYPE_2'])[['THLB_AREA_DECREASE']].sum().reset_index()
        ...
        df_rpdf_okn_s2['REDUCTION_FACTOR_S2'] = np.where(
            df_rpdf_okn_s2['OVERLAP_TYPE_2'] != 'IDF only', 
            1, 
        ...
        df_rpdf_okn_s2_sum = df_rpdf_okn_s2.groupby(['TSA_NAME', 'OVERLAP_TYPE_2'])[['THLB_AREA_DECREASE']].sum().reset_index()
        ...
        df_rpdf_kam = df_rpdf_kam[
            (df_rpdf_kam['OVERLAP_TYPE_2'] != 'IDF only') | 
            (~df_rpdf_kam['BEC_SUBZONE'].isin(['mw', 'mm']) | df_rpdf_okn['BEC_SUBZONE'].isnull())
        ]
        ...
        df_rpdf_kam_s1['REDUCTION_FACTOR_S1'] = np.where(
            df_rpdf_kam_s1['OVERLAP_TYPE_2'] != 'IDF only', 
            1, 
        ...
        df_rpdf_kam_s1_sum = df_rpdf_kam_s1.groupby(['TSA_NAME', 'OVERLAP_TYPE_2'])[['THLB_AREA_DECREASE']].sum().reset_index()
        ...
        df_rpdf_kam_s2['REDUCTION_FACTOR_S2'] = np.where(
            df_rpdf_kam_s2['OVERLAP_TYPE_2'] != 'IDF only', 
            1, 
        ...
        df_rpdf_kam_s2_sum = df_rpdf_kam_s2.groupby(['TSA_NAME', 'OVERLAP_TYPE_2'])[['THLB_AREA_DECREASE']].sum().reset_index()
    ...
    outfile= os.path.join(wks, 'outputs', f'{datetime}_summary_tables_total_THLB_decrease_v4_details.xlsx')

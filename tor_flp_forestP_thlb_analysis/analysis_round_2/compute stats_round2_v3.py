...
if __name__ == "__main__":
    ...
    try:
        ...
        df_rpdf_okn = df_rpdf_okn[
            (df_rpdf_okn['OVERLAP_TYPE'] != 'IDF only') | 
        ...
        ####### Kamloops scenarios #######
        df_rpdf_kam= dckCnx.execute("""SELECT* EXCLUDE geometry FROM r2_2_KAM_rip_idf_thlb_mdwr""").df()
        
        df_rpdf_kam= df_rpdf_kam.rename(columns={'TSA_NUMBER_DESCRIPTION': 'TSA_NAME'})
        df_rpdf_kam= df_rpdf_kam.rename(columns={'LEGAL_FEAT_PROVID': 'MDWR_OVERLAP'})
        
        df_rpdf_kam['THLB_AREA']= df_rpdf_kam['AREA_HA'] * df_rpdf_kam['thlb_fact']
        
        df_rpdf_kam= df_rpdf_kam[df_rpdf_kam['TSA_NAME']=='Kamloops TSA']
        ...
        df_rpdf_kam_s1 = df_rpdf_kam[
            (df_rpdf_kam['OVERLAP_TYPE'] != 'IDF only') | 
            ((df_rpdf_kam['PROJ_AGE_1'] >= 100) | df_rpdf_kam['PROJ_AGE_1'].isnull())
        ]
        
        
        df_rpdf_kam_s1 = df_rpdf_kam_s1[~((df_rpdf_kam_s1['OVERLAP_TYPE'] == 'IDF only') & (df_rpdf_kam_s1['PROJ_AGE_1'].isnull()))]
        

        df_rpdf_kam_s1['REDUCTION_FACTOR_S1'] = np.where(
            df_rpdf_kam_s1['OVERLAP_TYPE'] != 'IDF only', 
            1, 
            np.where(df_rpdf_kam_s1['MDWR_OVERLAP'].str.startswith('KAM'), 0.25, 0.5)
        )      
        ...
        df_rpdf_kam_s2['REDUCTION_FACTOR_S2'] = np.where(
            ...
            1, 
            np.where(df_rpdf_kam_s2['MDWR_OVERLAP'].str.startswith('KAM'), 0.25, 0.5)
        )      
        ...
        df_rpdfog_okn = df_rpdfog_okn[
            (df_rpdfog_okn['OVERLAP_TYPE'] != 'IDF only') | 
        ...
        ####### Kamloops scenarios #######
        df_rpdfog_kam= dckCnx.execute("""SELECT* EXCLUDE geometry FROM r2_2_KAM_rip_idf_ogda_thlb_mdwr""").df()
        
        
        df_rpdfog_kam= df_rpdfog_kam.rename(columns={'TSA_NUMBER_DESCRIPTION': 'TSA_NAME'})
        df_rpdfog_kam= df_rpdfog_kam.rename(columns={'LEGAL_FEAT_PROVID': 'MDWR_OVERLAP'})
        
        df_rpdfog_kam['THLB_AREA']= df_rpdfog_kam['AREA_HA'] * df_rpdfog_kam['thlb_fact']
        
        df_rpdfog_kam= df_rpdfog[df_rpdfog['TSA_NAME']=='Kamloops TSA']
        
        
        df_rpdfog_kam = df_rpdfog_kam[
            (df_rpdfog_kam['OVERLAP_TYPE'] != 'IDF only') | 
        ...
        df_rpdfog_kam_s1['REDUCTION_FACTOR_S1'] = np.where(
            ...
            1, 
            np.where(df_rpdfog_kam_s1['MDWR_OVERLAP'].str.startswith('KAM'), 0.25, 0.5)
        )      
    ...
    df_rpdfog_kam_s2.to_csv(os.path.join(wks, 'outputs', 'resultant_idf_rip_ogda_scenario2_kam_NEW.csv'), index=False)

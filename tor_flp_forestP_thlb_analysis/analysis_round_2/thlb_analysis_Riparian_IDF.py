...
def load_dck_sql():
    dkSql= {}
    '''  
    dkSql['r2_2_rip_idf_thlb']="""
         CREATE TABLE r2_2_rip_idf_thlb AS
             SELECT 
               thlb.TSA_NAME,
               rpdf.*,
               thlb.thlb_fact,
               ST_Area(ST_Intersection(rpdf.geometry, thlb.geometry)) / 10000.0 AS AREA_HA,
               ST_Intersection(rpdf.geometry, thlb.geometry) AS geometry
             ...
             JOIN 
                 thlb_tsa_qs thlb
             ON 
                 ST_Intersects(rpdf.geometry, thlb.geometry); 
         
        -- Fix geometry field name
        ALTER TABLE r2_2_rip_idf_thlb DROP COLUMN IF EXISTS geometry;  
        ALTER TABLE r2_2_rip_idf_thlb RENAME COLUMN geometry_1 TO geometry; 
        
        -- Build a spatial index
        CREATE INDEX idx_r2_2_r2_2_rip_idf_thlb ON r2_2_rip_idf_thlb USING RTREE (geometry);
                 """   
    '''             
    ########### MDWR intersection ##############  
    dkSql['r2_2_rip_idf_thlb_mdwr']="""
        CREATE TABLE r2_2_rip_idf_thlb_mdwr AS
            SELECT 
              thlb.*,
